...
import com.tt.dramatime.databinding.EpisodesMoreRankItemBinding
 ...
 * </pre>
 */
class EpisodesMoreRankAdapter(dataList: MutableList<MovieListBean>) :
    BaseVBQuickAdapter<EpisodesMoreRankItemBinding, MovieListBean>(dataList) {


    override fun onBindViewHolder(
        holder: BaseVBHolder<EpisodesMoreRankItemBinding>, position: Int, item: MovieListBean?
    ) {
        item?.apply {
            holder.binding.apply {
                ...
                tagList?.let {
                    tagList= if (it.size > 2) it.subList(0, 2) else tagList
